# ============================================================
# PROSTA GREDA - oslonci duz CIJELE krajnje ivice - kazneni pristup
#
#            q = 20 kN/m
#   | | | | | | | | | | | | | |
...
facets_lijevo = locate_entities_boundary(domen, fdim, lambda x: np.isclose(x[0], 0.0))
facets_desno  = locate_entities_boundary(domen, fdim, lambda x: np.isclose(x[0], L))
...
print(f"\nKazneni clan  : djeluje na {len(dofs_L_y[0])} DOF-ova lijevo, "
      f"{len(dofs_D_y[0])} desno  (cijela ivica)")
print(f"Blokirano ux  : {len(dofs_ux[0])} DOF  (samo tacka (0, h/2))")
# ============================================================
# 5) OPTERECENJE NA GORNJOJ IVICI I DEFINISANJE BOČNIH IVICA
# ============================================================
# oznaka 1 = gornja ivica (opterecenje)
# oznaka 2 = lijeva i desna krajnja ivica (oslonci)
...
poredak = np.argsort(svi)                 # meshtags trazi sortiran niz
...
kazna = fem.Constant(domen, np.float64(1.0e13))
...
# --- KONTROLA KAZNENOG PRISTUPA ---
# Kod Dirihleovog uslova je uy na osloncu tacno nula. Kod kaznenog
# pristupa nije, nego zavisi od faktora k - zato ga ispisujemo.
uy_osl = abs(vrijednost_u_tacki(uh, 0.0, h/2.0)[1])
print(f"\nUleknuce oslonca         : {uy_osl*1000:10.3e} mm")
...
with VTXWriter(domen.comm, "N1_oslonac_ivica_penalty.bp",
               [u1, sxx, sxy], engine="BP4") as vtx:
    vtx.write(0.0)

print("\nParaView izvoz: N1_oslonac_ivica_penalty.bp")